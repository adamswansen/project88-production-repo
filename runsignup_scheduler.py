...
class RunSignUpScheduler:
    ...
    def record_sync_attempt(self, timing_partner_id: int, status: str, details: str = None, 
                           events_synced: int = 0, participants_synced: int = 0):
        ...
        try:
            cursor.execute("""
                INSERT INTO sync_history (
                    timing_partner_id, provider_id, sync_type, status, 
                    sync_time, events_synced, participants_synced, details
                ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
            """, (
                timing_partner_id, 2, 'incremental', status, datetime.now(),
                events_synced, participants_synced, details
            ))
            conn.commit()
        ...
        try:
            ...
            for event in provider_events:
                try:
                    ...
                    if participants:
                        logger.info(f"👥 Found {len(participants)} updated participants for event {event.provider_event_id}")
